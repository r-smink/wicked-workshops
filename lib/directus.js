...
export async function directusFetch(path, options = {}) {
  ...
  try {
    const res = await fetch(url, {
      ...
      next: options.noCache ? undefined : { revalidate: 60 },
    });
    if (!res.ok) {
    ...
    let data = json.data;
    /* Directus geeft altijd een array terug bij collection queries.
       Bij &single of &limit=1 willen we het eerste element, niet de array. */
    if (Array.isArray(data)) {
      if (data.length === 0) return null;
      /* &limit=1 en &single geven het eerste element terug. De regex voorkomt
         dat &limit=100 (of 12, 150, ...) ten onrechte als limit=1 wordt gezien. */
      if (path.includes("&single") || /&limit=1(?=&|$)/.test(path)) {
        return data[0];
      }
    }
    return data;
...
export async function directusCreate(collection, body) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`Directus ${res.status} bij POST ${collection}:`, text);
      return null;
    }
    const json = await res.json();
    return json.data;
...
export async function directusUpdate(collection, id, body) {
  if (!isDirectusConfigured) return null;
  /* directus_users moet via /users/ endpoint, niet /items/directus_users/ */
  const endpoint = collection === "directus_users" ? "users" : `items/${collection}`;
  const url = `${DIRECTUS_URL.replace(/\/$/, "")}/${endpoint}/${id}`;
  try {
    const res = await fetch(url, {
      method: "PATCH",
      headers: {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      console.error(`Directus ${res.status} bij PATCH ${collection}/${id}:`, text);
      return null;
    }
    const json = await res.json();
    return json.data;
...
function mapWorkshop(w) {
  ...
  return {
    ...
    price_per_person: w.price_per_person ?? null,
    ...
    cityName: relName(w.city) || "",
    cityId: typeof w.city === "string" ? w.city : (w.city?.id || ""),
    category: relName(w.category) || w.category || "",
    categoryName: relName(w.category) || "",
    categoryId: typeof w.category === "string" ? w.category : (w.category?.id || ""),
...
function mapProvider(p) {
  ...
  return {
    ...
    lat: p.lat, lng: p.lng,
    city: typeof p.city === "string" ? p.city : p.city?.id || "",
    cityName: relName(p.city) || "",
...
const WORKSHOP_FIELDS = [
  ...
  "city", "city.name", "city.slug", "category", "category.name", "category.slug", "category.icon",
...
const PROVIDER_FIELDS = [
  ...
  "kvk_number", "vat_number", "city.name", "city.slug", "details", "user",
].join(",");
...
/* Statische pagina's (bedrijven, cadeaubon). Hergebruikt hetzelfde
   content-page schema als catch-all pagina's. Fallback blijft mock-data. */
...
/* Haal de provider op die hoort bij de ingelogde Directus user.
   userId is het user-id uit /users/me. Providers zijn gekoppeld
   via het `user` veld (M2O naar directus_users). */
export async function getProviderProfile(userId = null) {
  if (userId) {
    const data = await directusFetch(
      `/items/providers?filter[user]=${userId}&fields=${PROVIDER_FIELDS}&single`,
      { noCache: true }
    );
...
/* Maak een nieuwe provider-record aan voor een ingelogde user. */
export async function createProviderForUser(user, extra = {}) {
  const display_name = [user.first_name, user.last_name].filter(Boolean).join(" ") || user.email || "Nieuwe aanbieder";
  const slug = display_name.toLowerCase().trim()
    .replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-")
    || `provider-${Date.now()}`;
  return directusCreate("providers", {
    status: "draft",
    slug,
    display_name,
    user: user.id,
...
export async function getAgeRatings() {
  const data = await directusFetch(`/items/age_ratings?fields=id,name&limit=10`);
  if (data && data.length) {
...
export async function getPage(slug) {
  const requested = (slug || "").trim();
  const data = await directusFetch(
    `/items/pages?filter[slug]=${encodeURIComponent(requested)}&filter[status]=published&fields=${CONTENT_PAGE_FIELDS}&single`
  );
  if (data) return mapContentPage(data);
  return PAGE_FALLBACKS[requested] || PAGE_FALLBACKS.bedrijven;
}

/* ---------- Content-pagina's (catch-all /slug) ----------

   De `pages`-collectie bevat pagina's die in Directus worden aangemaakt
   (over ons, privacy, voorwaarden, etc.). Ze staan op root-niveau via
   app/[slug]/page.jsx. De collectie heeft bewust een klein schema
   (title, slug, body, seo_*) — rijkere blokken komen later via een
   M2A-sectiesstructuur. */
const CONTENT_PAGE_FIELDS = [
  "id", "slug", "title", "body", "seo_title", "seo_description", "seo_image", "status",
  "menu_label", "menu_location", "menu_order",
  "blocks.id", "blocks.type", "blocks.sort", "blocks.title", "blocks.body", "blocks.image",
  "blocks.items", "blocks.cta_label", "blocks.cta_to", "blocks.style",
  "blocks.column_left", "blocks.column_right", "blocks.video_url", "blocks.video_file",
].join(",");

const MENU_PAGE_FIELDS = ["id", "slug", "title", "menu_label", "menu_location", "menu_order"].join(",");
...
function mapContentBlock(b) {
  if (!b) return null;
  const imgId = imageId(b.image);
  const videoFileId = imageId(b.video_file);
  return {
    id: b.id,
    type: b.type || "text",
    sort: b.sort ?? 0,
    title: b.title ?? "",
    body: b.body ?? "",
    image: imgId ? assetUrl(imgId) : null,
    items: normalizeBlockItems(b.items),
    cta_label: b.cta_label ?? "",
    cta_to: b.cta_to ?? "",
    style: b.style || "coral",
    column_left: b.column_left ?? "",
    column_right: b.column_right ?? "",
    video_url: b.video_url ?? "",
    video_file: videoFileId ? assetUrl(videoFileId) : null,
  };
}

function mapContentPage(p) {
  if (!p || !p.slug) return p;
  return {
    slug: p.slug,
    title: p.title ?? "",
    hero_title: p.title ?? "",
    hero_subtitle: "",
    hero_image: p.seo_image ? assetUrl(p.seo_image) : null,
    intro: "",
    body: p.body ?? "",
    sections: [],
    ...
    seo_title: p.seo_title ?? "",
    seo_description: p.seo_description ?? "",
    /* Voor data-directus attributen (Visual Editor). */
    directus: { collection: "pages", item: p.id },
  };
}
...
export async function getContentPageSlugs() {
  const data = await directusFetch(
    `/items/pages?fields=slug&filter[status]=published&limit=100`
  );
  return Array.isArray(data) ? data.map((p) => p.slug).filter(Boolean) : [];
}

/* Menu-items voor header/footer. Haalt pagina's op waarvoor menu_location
   overeenkomt met de gevraagde locatie óf "both". */
export async function getMenuItems(location = "header") {
  const targets = [location, "both"].join(",");
  const data = await directusFetch(
    `/items/pages?fields=${MENU_PAGE_FIELDS}&filter[status]=published&filter[menu_location][_in]=${targets}&sort=menu_order&limit=50`
  );
  ...
  return data
    .map((p) => ({
      id: p.id,
      slug: p.slug,
      label: p.menu_label || p.title || p.slug,
      href: `/${p.slug}`,
...
/* Sessie ophalen met de workshop-velden die de checkout nodig heeft
   (prijs, instant_bookable, provider). noCache: plekken kunnen net
   vergeven zijn. */
export async function getSessionForCheckout(sessionId) {
  return directusFetch(
    `/items/workshop_sessions/${sessionId}?fields=id,starts_at,ends_at,capacity,seats_taken,status,price_override,workshop.id,workshop.slug,workshop.title,workshop.price_per_person,workshop.instant_bookable,workshop.provider`,
    { noCache: true }
  );
}

/* Booking op booking_code — voor de terugkeerpagina na betaling. */
export async function getBookingByCode(code) {
  const data = await directusFetch(
    `/items/bookings?filter[booking_code]=${encodeURIComponent(code)}&fields=id,booking_code,guest_name,participants,unit_price,total,status,payment_status,payment_id,session.starts_at,workshop.title,workshop.slug&single`,
    { noCache: true }
  );
  return data || null;
}

/* Booking op id — voor de Mollie-webhook en provider-acties. */
export async function getBookingById(id) {
  return directusFetch(
    `/items/bookings/${id}?fields=id,booking_code,guest_name,guest_email,participants,unit_price,subtotal,total,status,payment_status,payment_id,session,session.seats_taken,workshop,workshop.title,provider,customer`,
    { noCache: true }
  );
}
