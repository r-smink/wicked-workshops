...
export async function getWorkshop(slug) {
  const data = await directusFetch(
    `/items/workshops?filter[slug]=${slug}&fields=${WORKSHOP_FIELDS}&single`
  );
...
export async function getArticle(slug) {
  const data = await directusFetch(`/items/articles?filter[slug]=${slug}&fields=${ARTICLE_FIELDS}&single`);
  if (!data) return ARTICLES.find((a) => a.slug === slug) || ARTICLES[0];

  const mapped = mapArticle(data);
  const faqData = await directusFetch(
    `/items/article_faq?filter[article]=${data.id}&fields=${ARTICLE_FAQ_FIELDS}&sort=sort`
  );
  mapped.faq = faqData?.length ? faqData.map(mapFaq) : (mapped.faq?.length ? mapped.faq : ART_FAQ);
  return mapped;
}
