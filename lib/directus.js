...
export async function getCategories() {
  const data = await directusFetch("/items/categories?fields=*,workshop_count&sort=sort&limit=20");
  return data ? data.map(mapCategory) : CATEGORIES;
}

export async function getFeatured() {
  const data = await directusFetch(
    "/items/workshops?filter[featured]=true&limit=4" +
    "&fields=*,city.name,city.slug,category.name,category.slug,category.icon"
  );
...
export async function getWorkshop(slug) {
  const data = await directusFetch(
    `/items/workshops?filter[slug]=${slug}&single` +
    "&fields=*,city.name,city.slug,category.name,category.slug,category.icon,provider.display_name,provider.slug"
  );
...
export async function getSessions(workshopId) {
  const data = await directusFetch(
    `/items/workshop_sessions?filter[workshop][slug]=${workshopId}&sort=starts_at&limit=20`
  );
...
export async function getReviews(workshopId) {
  const data = await directusFetch(
    `/items/reviews?filter[workshop][slug]=${workshopId}&limit=4`
  );
...
export async function getFaq(workshopId) {
  const data = await directusFetch(
    `/items/workshop_faq?filter[workshop][slug]=${workshopId}&sort=sort`
  );
...
export async function getProviderWorkshops(providerId) {
  const data = await directusFetch(
    `/items/workshops?filter[provider][slug]=${providerId}` +
    "&fields=*,city.name,city.slug,category.name,category.slug,category.icon"
  );
...
export async function getProviderReviews(providerId) {
  const data = await directusFetch(
    `/items/reviews?filter[provider][slug]=${providerId}&limit=4`
  );
...
export async function getDashboardSessions() {
  ...
  return data ? data.map(mapSession) : DASH_SESSIONS;
}
...
export async function getDashboardWorkshops() {
  const data = await directusFetch(
    "/items/workshops?limit=20" +
    "&fields=*,city.name,city.slug,category.name,category.slug,category.icon"
  );
  return data ? data.map(mapWorkshop) : DASH_WORKSHOPS;
}
