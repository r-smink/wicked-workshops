...
function providerToForm(provider = null, user = null) {
  ...
  return {
    ...EMPTY_PROFILE,
    first_name: user?.first_name || provider?.first_name || d.first_name || "",
    last_name: user?.last_name || provider?.last_name || d.last_name || "",
    email: user?.email || provider?.email || d.email || "",
    phone: d.phone || provider?.phone || "",
    birth_date: d.birth_date || provider?.birth_date || "",
    street: d.billing_street || provider?.street || "",
    house_number: d.billing_house_number || provider?.house_number || "",
    addition: d.billing_addition || provider?.addition || "",
    postal_code: d.billing_postal_code || provider?.postal_code || "",
    residence: d.billing_residence || provider?.residence || "",
    country: d.billing_country || provider?.country || "Nederland",
    company_name: d.company_name || provider?.company_name || "",
    kvk_number: provider?.kvk_number || d.kvk_number || "",
    vat_number: provider?.vat_number || d.vat_number || "",
    vat_liable: d.vat_liable ?? provider?.vat_liable ?? true,
    kor: d.kor ?? provider?.kor ?? false,
    display_name: provider?.display_name || d.display_name || [user?.first_name, user?.last_name].filter(Boolean).join(" ") || "",
    profession: provider?.profession || d.profession || "",
    bio_short: provider?.bio_short || d.bio_short || "",
    bio_long: provider?.bio_long || d.bio_long || "",
    languages: Array.isArray(d.languages) ? d.languages : (Array.isArray(provider?.languages) ? provider.languages : ["Nederlands"]),
    ...
    location_street: d.location_street || provider?.location_street || "",
    location_postal: d.location_postal || provider?.location_postal || "",
    wheelchair: d.wheelchair ?? provider?.wheelchair ?? false,
    parking: d.parking ?? provider?.parking ?? false,
    transit: d.transit ?? provider?.transit ?? false,
    accepts_groups: provider?.accepts_groups ?? d.accepts_groups ?? true,
    max_group_size: String(provider?.max_group_size || d.max_group_size || "12"),
    iban: d.iban || provider?.iban || "",
    account_holder: d.account_holder || provider?.account_holder || "",
    terms: d.terms ?? provider?.terms ?? false,
  };
}
